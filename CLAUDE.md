...
탑뷰 스프라이트는 개미이며, 뒤에 남는 것도 개미 발자국이다.
...
개미를 탭하면 추가 매수 시트가 열린다. 계산은 `averageIn()` 하나이고
...
**시세는 아직 목 데이터다.** 목 엔진은 `packages/shared`에 있다 — API(Node)와
단일 HTML 목업(브라우저)이 **같은 엔진**을 돌리기 위해서다. 실제 시세를 붙일 때는
