 ...
 *
 * **반등 구간은 장대 양봉만 쭉 세운다.** 여기 섞인 짧은 봉을 두면 앞 6초가 "오르긴 하는데
 * 주춤거리는" 화면이 되어, 관객이 개미와 같이 신나지 않는다 — 속아야 반전이 산다. 값은
 * 12에서 42까지밖에 안 오르지만(고점은 102였다) **확대하면 창이 그 구간만 담아** 화면을
 * 가득 채우는 상승으로 보인다. 그게 이 판의 속임수다.
 */
const BNC_CHART: readonly number[] = [
  102, 93, 92, 94, 84, 82, 83, 75, 74, 73, 76, 65, 63, 64, 57, 55, 57, 48, 47, 46, 48, 40, 37, 38,
  28, 27, 29, 22, 20, 19, 21, 12, 11, 12, 15, 20, 24, 30, 35, 42,
];
