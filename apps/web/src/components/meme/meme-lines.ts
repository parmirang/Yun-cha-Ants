 ...
 * 달리 여기는 **버티는 중**이다.
 *
 * 풀은 **두 목소리를 섞는다.** 하나는 울 자격이 없다고 우기는 허세("프로는 안 울어"),
 * 하나는 그 허세가 새는 소리 — 차트한테 직접 대는 짧은 떼("왜 자꾸 떨어져"). 한쪽만
 * 두면 판이 밋밋하다: 허세만이면 표정과 말이 한 방향으로 굳고, 떼만이면 그냥 우는
 * 개미가 되어 **제목("안 울어")이 거짓말이 된다.** 둘이 섞여야 참다가 새는 얼굴이 된다.
 *
 * **줄끼리 순서를 전제하지 말 것** — 한 바퀴에 세 줄이 seed로 밀려 나오므로, 무너지는
 * 순서(허세 → 떼)를 짜봐야 어느 줄이 먼저 뜰지 모른다. 각 줄이 혼자 서야 한다.
 *
 * 떼는 **짧고 반말로** 둔다. 존댓말("내려주세요 제발")은 여러 마리가 함께 지르는
 * 롤러코스터·로켓 쪽 목소리다 — 여기는 책상 앞 혼잣말이라 상대가 차트 하나뿐이다.
 */
